...
auto failure(const char* message) {
    return std::unexpected(common::Error{message});
}
}  // namespace
...
common::Result<void> save_count(const char* directory, int count) {
    if (count < 0 || count > 999999)
        return failure("The count is outside its supported range");
    common::Owner<PendingFile> temporary(new (std::nothrow) PendingFile);
    if (!temporary)
        return failure("Cannot allocate saved-count state");
    ...
    for (int offset = 0; offset < length;) {
        ...
        if (written < 0 && errno == EINTR)
            continue;
        if (written <= 0)
            return failure("Cannot write the saved count");
        offset += written;
    }
    if (fsync(temporary->fd) != 0)
        return failure("Cannot flush the saved count");
    int closed = close(temporary->fd);
    temporary->fd = -1;
    if (closed != 0)
        return failure("Cannot close the saved count");
    ...
    if (dir < 0)
        return failure("Cannot flush the saved-count directory");
