...
int main() {
  ...
  for (unsigned version : {1u, 2u}) {
    ...
    if (!restored || restored->count != 20 || !restored->maximum || !restored->paused ||
        ...
        restored->zoom != (version == 1 ? 1.f : previous.zoom) || restored->bird != Bird::kPelican)
      return 6;
  }
  ...
  if (!restored || restored->bird != Bird::kPelican) return 8;
  previous[12] |= std::byte{4};
  if (decode_state(previous)) return 9;
  ...
  for (std::size_t length = 0; length < bytes.size(); ++length)
    if (length != sizeof(legacy) && decode_state({bytes.data(), length})) return 3;
