...
void apply_key(Runtime& r, Key action) {
  ...
  int index = -1;
  for (int i = 0; i < 5; ++i)
    if (order[i] == r.state.focused) index = i;
...
void apply_touch(Runtime& r, const Command& command) {
  ...
  switch (static_cast<Touch>(command.value)) {
    ...
    case Touch::kMove: {
      ...
      if (moved || hit != r.state.pressed) r.state.pressed = Control::kNone;
      break;
    }
    case Touch::kUp:
...
Result<void> draw(Runtime& r, bool wait) {
  if (!r.scene || !r.state.error.empty()) return {};
  r.dirty_visual = true;
  ...
  if (wait || r.redraw_sequence) {
    if (auto waited = gpu::wait_frame(*r.renderer); !waited) return waited;
  }
...
void record_result(Runtime& r, const Result<void>& result) {
  if (!result) r.state.error = result.error().message;
}
...
void check_surface(Runtime& r) {
  if (!needs_surface_check(r)) return;
  ...
  for (auto& command : commands) {
    switch (command.kind) {
      case CommandKind::kSurface:
        if (r.state.error.empty()) record_result(r, create_surface(r, command));
      ...
      case CommandKind::kVisible:
        ...
        if (r.visible) r.dirty_visual = true;
        break;
...
void* worker(void* data) {
  ...
  while (!r.stopping) {
    ...
    if (needs_surface_check(r)) {
      ...
      if (!next_surface_check) next_surface_check = now + kSurfaceCheckMilliseconds;
...
Result<void> synchronize(Runtime& r, CommandKind kind) {
  ...
  if (r.deadline_failed) deadline = r.recovery_deadline;
  auto pending = [&] {
    return !r.exited && (kind == CommandKind::kStop || r.completed_sequence < sequence ||
  ...
  while (pending()) {
    ...
    if (result && pending()) {
      ...
      if (releasing)
        stop_unresponsive_worker(kind == CommandKind::kStop ? "stop" : "release its window");
...
Result<Owner<Runtime>> create_runtime(const char* directory, SessionState restored) {
  ...
  if (!runtime) return std::unexpected(Error{"Cannot allocate application runtime"});
  ...
  if (!error) error = pthread_cond_init(&runtime->completed_condition, &attributes);
  pthread_condattr_destroy(&attributes);
  if (error) return std::unexpected(Error{"Cannot initialize runtime synchronization"});
  ...
  if (!error) error = pthread_create(&runtime->thread, &thread_attributes, worker, runtime.get());
  pthread_attr_destroy(&thread_attributes);
  if (error) return std::unexpected(Error{"Cannot start the rendering thread"});
...
Result<void> set_surface(Runtime& r, ANativeWindow* window, int width, int height) {
  ...
  if (window) {
    ...
    if (!command.window) return std::unexpected(Error{"Cannot allocate window state"});
...
Result<void> detach_surface(Runtime& r) { return synchronize(r, CommandKind::kDetach); }

Result<void> redraw(Runtime& r) { return synchronize(r, CommandKind::kRedraw); }
...
void set_density(Runtime& r, float pixels_per_dp) {
  if (!std::isfinite(pixels_per_dp) || pixels_per_dp <= 0) return;
