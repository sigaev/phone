...
VKAPI_ATTR VkResult VKAPI_CALL __wrap_vkGetPhysicalDeviceSurfaceFormatsKHR(
    ...
    if (formats)
        formats[0] = {VK_FORMAT_R8G8B8A8_UNORM, VK_COLOR_SPACE_SRGB_NONLINEAR_KHR};
    *count = 1;
    return VK_SUCCESS;
}
...
VKAPI_ATTR VkResult VKAPI_CALL __wrap_vkCreateSwapchainKHR(VkDevice device,
    ...
    if (info->oldSwapchain)
        reinterpret_cast<Swapchain*>(info->oldSwapchain)->retired = true;
...
VKAPI_ATTR VkResult VKAPI_CALL __wrap_vkGetSwapchainImagesKHR(VkDevice, VkSwapchainKHR handle,
    ...
    if (images)
        for (unsigned i = 0; i < swapchain.images.size(); ++i)
            images[i] = swapchain.images[i].handle;
