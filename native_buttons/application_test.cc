...
Result<void> check_runtime(const char* path) {
  CHECK(save_count(path, 7), "Cannot seed application count");
  {
    ...
    for (int i = 0; i < 7; ++i) activate(r, Control::kAdd);
    ...
    set_touch_slop(r, 8);
    touch(r, Touch::kDown, 100, 650);
    for (int i = 0; i < 40; ++i) touch(r, Touch::kMove, 100 + i % 2, 650);
    touch(r, Touch::kUp, 100, 650);
    ...
    CHECK(redraw(r) && get_state(r).zoom == .5f, "Far zoom limit or invalid scale handling failed");
    pinch(r, 3);
    ...
    CHECK(get_state(r).safe.w == 720 && get_state(r).safe.h == 320 && get_state(r).zoom == 1.5f &&
              get_state(r).bird == Bird::kPelican,
...
int main() {
  ...
  if (result) result = check_runtime(directory.c_str());
