...
void update_pinch(Gestures& g, const AInputEvent& event, int excluded, bool scale, size_t sample) {
    ...
    if (scale && same_pair && g.span >= g.minimum_span)
        pinch(*g.runtime, span / g.span);
    g.span = span;
}
}  // namespace
...
common::Result<common::Owner<Gestures>> create_gestures(Runtime& runtime) {
    ...
    if (!gestures)
        return std::unexpected(common::Error{"Cannot allocate touch gestures"});
    return gestures;
}
...
void handle_motion(Gestures& g, const AInputEvent& event) {
    ...
    if (action == AMOTION_EVENT_ACTION_DOWN) {
        ...
        if (!g.multitouch)
            touch(*g.runtime, Touch::kUp, AMotionEvent_getX(&event, 0),
        ...
        if (count >= 2) {
            ...
            touch(*g.runtime, Touch::kMove, AMotionEvent_getX(&event, 0),
                  AMotionEvent_getY(&event, 0));
        }
