...
void destroy(AlignedAllocation* allocation) noexcept {
    delete allocation;
}
...
common::Result<void> expect_abort(Failure failure) {
    auto child = fork();
    if (child < 0)
        return std::unexpected(common::Error{"Cannot fork the runtime failure check"});
    if (child == 0) {
...
common::Result<void> check_runtime() {
    ...
    if (auto result = expect_abort(Failure::kAllocation); !result)
        return result;
    if (auto result = expect_abort(Failure::kStringLength); !result)
        return result;
...
int main() {
    ...
    std::puts(
        "Allocation failure, alignment, handlers, strings, vectors, clocks and aborts passed");
    return 0;
}
