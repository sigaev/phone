...
Vec3 operator*(Vec3 a, float scale) { return {a.x * scale, a.y * scale, a.z * scale}; }
...
Vec3 unit(Vec3 value) { return value * (1.f / std::fmax(length(value), .00001f)); }
...
Mat4 operator*(const Mat4& a, const Mat4& b) {
  ...
  for (int c = 0; c < 4; ++c)
    for (int r = 0; r < 4; ++r) {
      m.v[c * 4 + r] = 0;
      for (int k = 0; k < 4; ++k) m.v[c * 4 + r] += a.v[k * 4 + r] * b.v[c * 4 + k];
    }
  return m;
}

Mat4 transform(Vec3 p, Vec3 s, float roll) { return translation(p) * rotate_z(roll) * scale(s); }
